...
```sh
gnome-extensions install --force waywallen-gnome-<版本>-<架构>.zip
gnome-extensions enable org.waywallen.gnome@waywallen.io
```

注销重新登录以加载。

### Wayland layer-shell 客户端

对于暴露 `zwlr_layer_shell_v1` 的 Wayland compositor（例如 Hyprland、Sway、
Niri），从最新 release 下载
`waywallen-layer-shell-<版本>-<架构>.tar.gz`，然后：
...
> [!NOTE]  
> AppImage 版本的 waywallen 已经内置 waywallen-layer-shell。  
> waywallen 会负责管理它。
